...
#infile = "./data/nfp4_QH_aspect_3.5_field_strength.pickle"
#infile = "./data/nfp4_QH_aspect_5.5_field_strength.pickle"
infile = "./data/nfp4_QH_aspect_8.7_field_strength.pickle"
...
for ax,p in ((ax1,p1),(ax2,p2),(ax3,p3),(ax4,p4)):
  ...
  ## make a continuous colorbar
  #norm= matplotlib.colors.Normalize(vmin=p.cvalues.min(), vmax=p.cvalues.max())
  #sm = plt.cm.ScalarMappable(norm=norm, cmap = p.cmap)
  #sm.set_array([])
  ##ticks = np.around(np.linspace(np.min(p.levels),np.max(p.levels),4),2)
  ##ticks = p.levels[np.linspace(0,len(p.levels)-1,6).round().astype(int)]
